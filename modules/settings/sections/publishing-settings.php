...
class Publishing_Settings extends Setting_Class {
    ...
    public function register_settings() {
        ...
        add_settings_field(
            ...
            __('Rate Limit Monitoring', 'radle'), // This will be used as the label in the `th` element
        ...
        add_settings_field(
            ...
            __('Default Title Template', 'radle'),
        ...
        add_settings_field(
            ...
            __('Default Content Template', 'radle'),
    ...
    private function get_field_description($option_name) {
        $descriptions = [
            'radle_subreddit' => __('The target subreddit where posts will be published. Accepts the subreddit name (e.g., gbti_network), a full URL to the subreddit (e.g., https://www.reddit.com/r/GBTI_network/).', 'radle'),
            'radle_default_post_type' => __('Choose whether to post as a text post or a link post on Reddit. If post is selected, the Default Content Template below will control the default message sent to reddit when posting. This message can be edited manually before publishing.', 'radle'),
            'radle_default_title_template' => __('The default template for the Reddit post title. You can use placeholders like {post_title}. All available token placeholders should be listed below. If you have a SEO management plugin installed, there may be support for special title and description tokens.', 'radle'),
            'radle_default_content_template' => __('The default template for the Reddit post content. You can use placeholders like {post_excerpt} and {post_permalink}. All available token placeholders should be listed below. If you have a SEO management plugin installed, there may be support for special title and description tokens. Please check below for the available tokens.', 'radle'),
        ];
    ...
    public function render_available_tokens() {
        echo '<p style="font-style: italic;">' . __('The following tokens are supported inside templates:', 'radle') . '</p>';
        ?>
        <ul>
    ...
    public function render_enable_rate_limit_monitoring_field() {
        ...
        <select name="radle_enable_rate_limit_monitoring" id="radle_enable_rate_limit_monitoring" style="min-width: 150px;">
            <option value="yes" <?php selected($value, 'yes'); ?>><?php echo __('Yes', 'radle'); ?></option>
            <option value="no" <?php selected($value, 'no'); ?>><?php echo __('No', 'radle'); ?></option>
        </select>
