...
if (!$reddit_post_id && is_admin()) {
    echo '<p>' . __('No Reddit post associated with this WordPress post.', 'radle') . '</p>';
    return;
} elseif (!$reddit_post_id) {
    echo '<p class="nocomments">' . __('Comments disabled.', 'radle') . '</p>';
    return;
}
...
echo '<div id="radle-comments-container"><p>' . esc_html__('Loading comments...', 'radle') . '</p></div>';
...
if ($show_powered_by === 'yes') {
    ...
    echo ' <a href="'.$reddit_post_url.'" target="_blank" rel="nofollow" title="' . __('Powered by the Reddit API','radle') . '"><img src="' . RADLE_PLUGIN_URL . 'assets/images/powered-by-reddit.webp" ></a>';
    echo ' <a href="https://gbti.network/radle" target="_blank" rel="nofollow" class="powered-by-radle" title="' . __('Powered by Radle WordPress Plugin','radle') . ' ' . RADLE_VERSION .'">'. __('Radle','radle') . ' ' . RADLE_VERSION .'</a>';
    echo '</div>';
}